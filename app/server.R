...
shinyServer(function(input, output, session) {
    ...
    getData <- reactive({
        ...
        all_data #%>% mutate(country = c("DE")) %>% filter(!is.na(search_date))
    })


    # raw_data: ----
    # base access all data
    raw_data <- reactive({
    ...
    # id_data: ----
    # all data with id coloumn
    # this affects the size of the Hilbert Curve
    id_data <- reactive({
        req(input$party_selector)
        ...
        nmax <- max(i_data$gghid)
        limit <- 4 ^ (ceiling(log(nmax, base = 4)))


        i_data %>% dplyr::mutate(gghid = round(((gghid - 1) * limit)/nmax))
    })

    # selected_data: ----
    # also selects by date
    selected_data <- reactive({
        ...
        #my_date <- "2017-09-20"
        i_data %>%
            filter(search_date == my_date) %>%
            filter(keyword %in% input$party_selector)
    })

    # regions_data: ----
    # generates background data
    regions_data <- reactive({
        i_data <- id_data()

        regions <- i_data %>%
            select(gghid, domain, url) %>%
    ...
    # label_positions_data: ----
    # a hold the positions of the top domain labels
    label_positions_data <- reactive({
        i_data <- id_data()
        top_domains <- top_domains_reactive()
        upper_limit <- upper_limit_reactive()
    ...
    # upper_limit_reactive ----
    # range of ids from id_data
    upper_limit_reactive <- reactive({
        i_data <- id_data()
        upper_limit <- i_data %>% pull(gghid) %>% max()
        upper_limit
    })

    # top_domains_reactive ----
    # List the top n domains
    top_domains_reactive <- reactive({
        req(input$top_n_level)

        all_data <- id_data()

        top_count <- input$top_n_level
    ...
    output$ui_party_selector <- renderUI({
        ...
        shiny::selectInput(
            "party_selector",
            label = "Pick Search Term to visualize",
            choices = sort(party_list),
            selected = sort(party_list)[1],
            multiple = F,
            selectize = T
        )
    ...
    output$vis <- renderPlot({
        ...
        label_positions <- label_positions_data()
    ...
    output$zoomedview <- shiny::renderPlot({
        ...
        i_data <- id_data()
        regions <- regions_data() %>% arrange(gghid)
        upper_limit <- max(i_data$gghid)
        ...
        regions <- data.frame(gghid = 1:upper_limit) %>%
            left_join(regions)

        regions <- regions %>%
            select(-x, -y) %>%
            mutate(domain_number = zoo::na.locf0(domain_number)) %>%
            mutate(domain = zoo::na.locf0(domain)) %>%
            gghilbertstrings::create_coordinates(gghid) %>%
            filter(x >= xmin - 3 , x <= xmax + 3, y >= ymin - 3, y <= ymax + 3)

        # Fix diagonal lines
        # calculate rowwie cumulative difference, whenever < 0 then increase group and reset cumulativ diff
        ...
        plot_data <- plot_data_re() %>%
            filter(x >= xmin - 1 , x <= xmax + 1, y >= ymin - 1, y <= ymax + 1)
        ...
        ggplot(plot_data) +
            ...
            geom_path(
                data = regions,
                mapping = aes(x = x, y = y, color = domain_number,
                              group = "1"
                              ),
                alpha = alpha_val
                #size = size_bkg_val
            ...
            coord_fixed(ylim = c(ymin, ymax), xlim = c(xmin, xmax)) +
        ...
        if (input$show_labels) {
            p <- p +
                ggrepel::geom_label_repel(
                    data = label_positions %>% filter(x >= xmin , x <= xmax, y >= ymin, y <= ymax),
                    mapping = aes(x = x, y = y, label = domain),
                    seed = 123,
                    size = txt_size,
                    show.legend = F,
    ...
    output$info <- renderDT({
        ...
        brushedPoints(regions, input$vis_brush) %>%
            ...
            select(link, rank, url) %>%
            arrange(desc(rank)) %>%
            mutate(rank = round(rank, digits = 1))
    ...
    output$tooltip <- shiny::renderTable({
        regions <- plot_data_re()
        # With base graphics, need to tell it what the x and y variables are.
        outputvariable <- nearPoints(regions, input$vis_hover, threshold = 15)
        outputvariable2 <- nearPoints(regions, input$vis2_hover, threshold = 15)
        #outputvariable <- brushedPoints(regions, input$vis_brush) %>% pull(url)

        outputvariable <- bind_rows(outputvariable,outputvariable2)

        outputvariable %>% select(keyword, rank, url)
    })
