...
fullplot <-
  singleplot1 + singleplot2 + plot_annotation(
    title = "Hilbert Plot for top search results for AfD and CDU",
    caption = paste(
      "Results for 1298 unique users. Individual results use 0.01 alpha blending."
    )
...
ggsave(plot = fullplot, filename = here("data", "sidebyside.pdf"), width = 8, height = 5)
```
...
```{r comparison}
temp_data <- all_data %>% 
  filter(search_date == dmy("20092017"))
...
tsne_out <- Rtsne(as.matrix(prep2)) # Run TSNE

tsne_out$Y %>% 
  as_tibble() %>% 
  add_column(party = prep2$keyword, rank = prep2$rank) %>% 
  ggplot() +
  aes(x = V1, y = V2, color = factor(party), size = rank) +
  geom_point()
